...
public class ConcurrentHashMapRrmark<K,V> extends AbstractMap<K,V>
     ...
     * 而且混合后的低位参杂了高位的部分特征，这样高位的信息也被变相保存下来。
     *
     * 采用扰动算法，碰撞大大减少。
     *
     * @param h
     ...
     * 返回c所对应的最大二进制数 + 1。
     *
     * 前面是计算c所对应的最大二进制数，最后返回 + 1，相当于二进制又增加了以位
     *
     * 参考 com.adeng.java.util.concurrent.BinaryTest#test1()
     *
     */
     ...
     * 创建一个新的空map，默认初始化，表大小(16)
     */
    public ConcurrentHashMapRrmark() {
    }

    public ConcurrentHashMapRrmark(int initialCapacity) {
     ...
     * @param m the map
     */
    public ConcurrentHashMapRrmark(Map<? extends K, ? extends V> m) {
        this.sizeCtl = DEFAULT_CAPACITY;
        putAll(m);
    }

    public ConcurrentHashMapRrmark(int initialCapacity, float loadFactor) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K,V> e = f;; ++binCount) {
                                //遍历节点的Key
                                K ek;
                                //如果hash相等，并且  Key相同或者 key.equals相同
                                if (e.hash == hash &&
                                        ((ek = e.key) == key || (ek != null && key.equals(ek)))) {
                                    //老的Value赋值
                                    oldVal = e.val;
                                    //如果onlyIfAbsent = false，则会替换value,默认是false。
                                    if (!onlyIfAbsent)
                                        e.val = value;
                                    break;
                                }
                                //如果hash相同，Key不同，则往当前节点的尾部添加节点
                                Node<K,V> pred = e;
                                if ((e = e.next) == null) {
    ...
    public static <K> KeySetView<K,Boolean> newKeySet() {
        return new KeySetView<K,Boolean>
                (new ConcurrentHashMapRrmark<K,Boolean>(), Boolean.TRUE);
    }
    ...
    public static <K> KeySetView<K,Boolean> newKeySet(int initialCapacity) {
        return new KeySetView<K,Boolean>
                (new ConcurrentHashMapRrmark<K,Boolean>(initialCapacity), Boolean.TRUE);
    }
     ...
     * 尝试扩容table
     *
     * @param size  当前table size 左移一位的大小
     */
    ...
    static class BaseIterator<K,V> extends Traverser<K,V> {
        final ConcurrentHashMapRrmark<K,V> map;
        Node<K,V> lastReturned;
        BaseIterator(Node<K,V>[] tab, int size, int index, int limit,
                     ConcurrentHashMapRrmark<K,V> map) {
    ...
    static final class MapEntry<K,V> implements Entry<K,V> {
        ...
        V val;       // non-null
        final ConcurrentHashMapRrmark<K,V> map;
        MapEntry(K key, V val, ConcurrentHashMapRrmark<K,V> map) {
            this.key = key;
            this.val = val;
            this.map = map;
        }
    ...
    static final class EntrySpliterator<K,V> extends Traverser<K,V>
        ...
        final ConcurrentHashMapRrmark<K,V> map; // To export MapEntry
        long est;               // size estimate
        EntrySpliterator(Node<K,V>[] tab, int size, int index, int limit,
                         long est, ConcurrentHashMapRrmark<K,V> map) {
            super(tab, size, index, limit);
            this.map = map;
            this.est = est;
        }
    ...
    abstract static class CollectionView<K,V,E>
        ...
        private static final long serialVersionUID = 7249069246763182397L;
        final ConcurrentHashMapRrmark<K,V> map;
        CollectionView(ConcurrentHashMapRrmark<K,V> map)  { this.map = map; }
         ...
         */
        public ConcurrentHashMapRrmark<K,V> getMap() { return map; }
    ...
    public static class KeySetView<K,V> extends CollectionView<K,V,K>
        ...
        KeySetView(ConcurrentHashMapRrmark<K,V> map, V value) {  // non-public
            super(map);
            this.value = value;
        }
        ...
        public Iterator<K> iterator() {
            Node<K,V>[] t;
            ConcurrentHashMapRrmark<K,V> m = map;
    ...
    static final class ValuesView<K,V> extends CollectionView<K,V,V>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        ValuesView(ConcurrentHashMapRrmark<K,V> map) { super(map); }
    ...
    static final class EntrySetView<K,V> extends CollectionView<K,V,Entry<K,V>>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        EntrySetView(ConcurrentHashMapRrmark<K,V> map) { super(map); }
    ...
    static {
        try {
            U = sun.misc.Unsafe.getUnsafe();
            Class<?> k = ConcurrentHashMapRrmark.class;
